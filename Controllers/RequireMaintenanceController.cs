    ...
    public async Task<IActionResult> ScheduleWithRequire([FromBody] OptionItemMaintananceSchedule Schedule)
    {
      ...
      try
      {
        ...
        if (GetDataTemp.Any())
        {
          ...
          DateTime? MinDate = ListDate.Where(x => x.Value != null // && x.Value.Year > (DateTime.Today.Year - 3)
          ).Min();
          DateTime? MaxDate = ListDate.Where(x => x.Value != null // && x.Value.Year < (DateTime.Today.Year + 1)
          ).Max();
